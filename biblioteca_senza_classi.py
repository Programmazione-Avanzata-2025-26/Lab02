...
def carica_da_file(file_path):
    ...
    for riga in righe:
        ...
        if(len(riga) == 1):
            ...
            libro = [titolo.strip(), autore.strip(), int(anno), int(numPagine)] #, int(sezione)
...
def aggiungi_libro(biblioteca, titolo, autore, anno, pagine, sezione, file_path):
    ...
    try:
        f = open(file_path, 'a')
        csv.writer(f).writerow([libro[0], libro[1], libro[2], libro[3], sezione])
...
def cerca_libro(biblioteca, titolo):
    ...
    for sezione in biblioteca: # sezione è una lista di libri
        for libro in sezione:
            if libro[0] == titolo:
                return f"{libro[0]} {libro[1]} {libro[2] } {libro[3]}"
...
def elenco_libri_sezione_per_titolo(biblioteca, sezione):
    ...
    titoli = [ libro[0] for libro in biblioteca[sezione-1] ]
